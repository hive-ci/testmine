class ResultHistory
  ...
  def self.find(args)
    ...
    (test_definition_id = args[:test_definition_id]) || raise('Need to provide a test definition id')
  ...
  def self.find_for_recent_targets(args)
    ...
    targets = Run.joins(results: :test_definition)
                 .where(results: { test_definition_id: test_definition_id })
                 .limit(200)
                 .order('runs.id DESC')
                 .pluck('DISTINCT runs.target')

    targets.push(target).uniq! if target

    # Finally find the results
    targets.each_with_object({}) { |t, h| h[t] = ResultHistory.find(args.merge(target: t)); }
  end
  ...
  def child_test_definitions
    primary_children.collect(&:test_definition)
  end

  def primary_child(test_definition)
    primary_children.detect { |c| c.test_definition.id == test_definition.id }
  end
