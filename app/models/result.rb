class Result < ActiveRecord::Base
  ...
  def count(status)
    ...
    unless @count[status]
      @count[status] = children.collect { |c| c.calculated_status == status.to_s }.count(true)
    end
    @count[status]
  end
  ...
  def calculated_status
    unless @calculated_status
      @calculated_status = status ? status : Result.summary_status(children.collect(&:calculated_status))
    end
    @calculated_status
  end
