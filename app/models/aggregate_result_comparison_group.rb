class AggregateResultComparisonGroup
  ...
  def self.populate(args)
    (primary_world_id = args[:primary_world_id]) || raise('Need to provide a primary world id')
    (reference_world_id = args[:reference_world_id]) || raise('Need to provide a reference world id')
  ...
  #
  def status
    AggregateResultComparisonGroup.summary_status(results.collect(&:status))
  end

  def diff
    results.collect(&:diff).any? { |v| v == true }
  end
