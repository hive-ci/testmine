...
class IrIngestor
  def self.parse_ir(json)
    ...
    (world = process_world(project, component, version)) || raise("Couldn't derive a world")
    ...
    (run = Run.create(started_at: started,
                      ...
                      world_id: world.id)) || raise("Couldn't create a new run")
    #
    # 3) Process the suite
    #
    suite = Suite.find_or_create(project: project,
                                 name:    suite,
                                 runner:  type)
    ...
    (results = process_results(run, suite, results, suite)) || raise("Couldn't process results")

    run.status = Result.summary_status(results.collect(&:status))
    run.save
    run
  end
    ...
    if results_array.respond_to?(:each)
      results_array.each do |r|
        ...
        if children
          ...
          unless status
            result.status = Result.summary_status(child_results.collect(&:status))
